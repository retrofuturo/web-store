...
* Slicknav для создания мобильного меню;
* Carousel bootstrap для карусели на главной странице;
* Flexslider для карусели на странице карточки товара;
* Formstyler для стилизации форм.

Используются `иконки` font awesome, а также сделанные при помощи sprite.

## Default components

* `.title` - используется для средних заголовков;
* `.title.big` для крупных;
* `.default-section` определяет стандартные отступы для секций;
* `.subtitle` - стандартный подзаголовок;
* `.item-product` - маленькая карточка товара;

### Кнопки

* `.btn.btn-default` - стандартная кнопка;
* `.btn.btn-round` - скругленная.

#### Большая карточка товара

* `.items-text span` - подзаголовок;
* `.items-text p` - стандартный текст.

## Default classes

* `.flex-container` - задаёт display-flex, образуя флекс-контейнер;
* `.align-center` - выравнивает флекс-элементы по вертикали;
* `.justify-sp-between` - выравнивает флекс-элементы по ширине;
* `.orange-text` - задаёт оранжевый цвет тексту.
